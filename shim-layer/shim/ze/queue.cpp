...
ze_result_t shim::ze::Queue::Execute(const shim::ze::Command &cmd) {
  for (auto &event : cmd.wait_) {
    ...
    if (auto r = SHIM_CALL(zeEventHostSynchronize)(h, ~0ull);
        r != ZE_RESULT_SUCCESS) {
  ...
  case shim::ze::Command::Type::EventReset:
    if (auto r = SHIM_CALL(zeEventHostReset)(
            reinterpret_cast<ze_event_handle_t>(cmd.events_[0].get()));
        r != ZE_RESULT_SUCCESS) {
      return ZE_RESULT_ERROR_DEVICE_LOST;
    }
    break;
    ...
    for (size_t i = 0; i < cmd.events_.size(); i++) {
      ...
      if (cmd.timestamps_.offsets) {
        dst = reinterpret_cast<ze_kernel_timestamp_result_t *>(
            static_cast<char *>(cmd.timestamps_.dst) +
            cmd.timestamps_.offsets[i]);
      } else {
        dst = &reinterpret_cast<ze_kernel_timestamp_result_t *>(
            cmd.timestamps_.dst)[i];
      }

      auto r = SHIM_CALL(zeEventQueryKernelTimestamp)(
          reinterpret_cast<ze_event_handle_t>(cmd.events_[i].get()), dst);
  ...
  case shim::ze::Command::Type::ImageCopyFromMem: {
    auto r = std::visit(
        [src = cmd.copy_.src](auto &surf) {
          return surf->WriteSurface(reinterpret_cast<const uint8_t *>(src),
                                    nullptr);
        },
        cmd.image_->surface_);
    if (r != CM_SUCCESS) {
  ...
  case shim::ze::Command::Type::Kernel: {
    ...
    if (auto r = dev_->CreateThreadGroupSpace(
            threads.groupSizeX, threads.groupSizeY,
            cmd.group_count_.groupCountX, cmd.group_count_.groupCountY, tgs);
        r != CM_SUCCESS) {
      return ZE_RESULT_ERROR_DEVICE_LOST;
    }
...
SHIM_CALL(zeCommandQueueDestroy)(ze_command_queue_handle_t hCommandQueue) {
  ...
  shim::IntrusivePtr<shim::ze::Queue> queue(
      reinterpret_cast<shim::ze::Queue *>(hCommandQueue), false);
...
ZE_APIEXPORT ze_result_t ZE_APICALL SHIM_CALL(
  ...
  for (size_t i = 0; i < numCommandLists; i++) {
    shim::IntrusivePtr<shim::ze::CommandList> cmdlist(
        reinterpret_cast<shim::ze::CommandList *>(phCommandLists[i]));
