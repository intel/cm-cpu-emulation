...
Image::Image(IntrusivePtr<CmDeviceEmu> dev, uint32_t size)
    : type_(Image::Type::kBuffer), surface_(std::in_place_type<BufferT>, nullptr, Deleter(dev)) {
...
Image::Image(IntrusivePtr<CmDeviceEmu> dev, uint32_t width, uint32_t height, CM_SURFACE_FORMAT format)
    : type_(Image::Type::k2D), surface_(std::in_place_type<Surface2DT>, nullptr, Deleter(dev)) {
  CmSurface2D *surf = nullptr;
  if (auto r = dev->CreateSurface2D(width, height, format, surf); r != CM_SUCCESS) {
    throw Error(::GetCmErrorString(r));
  }
...
Image::Image(IntrusivePtr<CmDeviceEmu> dev, uint32_t width, uint32_t height, uint32_t depth, CM_SURFACE_FORMAT format)
    : surface_(std::in_place_type<Surface3DT>, nullptr, Deleter(dev)) {
  CmSurface3D *surf = nullptr;
  if (auto r = dev->CreateSurface3D(width, height, depth, format, surf); r != CM_SUCCESS) {
    throw Error(::GetCmErrorString(r));
  }
...
SurfaceIndex *Image::GetIndex() {
  return std::visit([](auto &p) -> SurfaceIndex* {
  ...
  try {
    ...
    switch (desc->type) {
      case ZE_IMAGE_TYPE_2D:
        ...
        image = shim::MakeIntrusive<shim::ze::Image>(dev, desc->width, desc->height, fmt);
        break;
      case ZE_IMAGE_TYPE_3D:
        if (fmt == CM_SURFACE_FORMAT_UNKNOWN) {
          return ZE_RESULT_ERROR_UNSUPPORTED_IMAGE_FORMAT;
        }
        image = shim::MakeIntrusive<shim::ze::Image>(dev, desc->width, desc->height, desc->depth, fmt);
        break;
      case ZE_IMAGE_TYPE_BUFFER:
...
ZE_APIEXPORT ze_result_t ZE_APICALL SHIM_CALL(zeImageDestroy)(
  ...
  shim::IntrusivePtr<shim::ze::Image> image(reinterpret_cast<shim::ze::Image*>(hImage), false);
