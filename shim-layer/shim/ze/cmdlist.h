...
class Command {
 public:
  ...
  Command(void *dst, const void *src, size_t size,
          IntrusivePtr<Event> signal, std::vector<IntrusivePtr<Event>> &&wait)
      : type_(Type::BufferCopy), copy_({size, dst, src}), signal_(signal), wait_(std::move(wait)) {}

  Command(void *dst, uint8_t pattern, size_t size,
          IntrusivePtr<Event> signal, std::vector<IntrusivePtr<Event>> &&wait)
      : type_(Type::BufferFill), fill_({size, dst, pattern}), signal_(signal), wait_(std::move(wait)) {}
  ...
  Command(void *dst, IntrusivePtr<Image> image,
          IntrusivePtr<Event> signal, std::vector<IntrusivePtr<Event>> &&wait)
  ...
  Command(std::vector<IntrusivePtr<Event>> &&events, void *dst, const size_t *offsets,
          IntrusivePtr<Event> signal, std::vector<IntrusivePtr<Event>> &&wait)
...
class CommandList : public IntrusiveRefCounter<CommandList> {
 public:
  CommandList(IntrusivePtr<Context> context, IntrusivePtr<Queue> queue = nullptr)
      : context_(context), queue_(queue) {}

  ze_result_t Append(const Command &cmd);

  template <typename ... Args>
  ze_result_t Emplace(Args &&...args) {
    if (queue_) {
