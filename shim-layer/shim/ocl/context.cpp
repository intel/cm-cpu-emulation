...
CL_API_ENTRY cl_context CL_API_CALL SHIM_CALL(clCreateContext)(
...
CL_API_ENTRY cl_context CL_API_CALL SHIM_CALL(clCreateContextFromType)(
  ...
  cl_uint num_devs = 0;
  if (auto status =
          SHIM_CALL(clGetDeviceIDs)(nullptr, device_type, 1, &dev, &num_devs);
      status != CL_SUCCESS) {
    ERRCODE(status);
    return nullptr;
  }

  return SHIM_CALL(clCreateContext)(properties, 1, &dev, pfn_notify, user_data,
                                    errcode_ret);
}
...
CL_API_ENTRY cl_int CL_API_CALL SHIM_CALL(clReleaseContext)(cl_context context)
    CL_API_SUFFIX__VERSION_1_0 {
  shim::IntrusivePtr<shim::cl::Context> ctx(
      static_cast<shim::cl::Context *>(context), false);
  return CL_SUCCESS;
}

CL_API_ENTRY cl_int CL_API_CALL SHIM_CALL(clGetContextInfo)(
  ...
  case CL_CONTEXT_DEVICES:
    return SetResult<cl_device_id>(&ctx->dev_, param_value_size, param_value,
                                   param_value_size_ret);
...
CL_API_ENTRY cl_int CL_API_CALL SHIM_CALL(clSetContextDestructorCallback)(
    cl_context context,
