...
SHIM_CALL(clCreateProgramWithSource)(cl_context        context,
  ...
  shim::IntrusivePtr<shim::cl::Program> prog = new(std::nothrow) shim::cl::Program(ctx, std::move(srcs));
...
SHIM_CALL(clCreateProgramWithBinary)(cl_context                     context,
  ...
  CmProgram *p = nullptr;
  if (ctx->dev_.device->LoadProgram(const_cast<unsigned char *>(binaries[0]), lengths[0], p) != CM_SUCCESS) {
    ERRCODE(CL_INVALID_BINARY);
    return nullptr;
  }

  shim::IntrusivePtr<shim::cl::Program> prog = new(std::nothrow) shim::cl::Program(ctx, p);
...
SHIM_CALL(clReleaseProgram)(cl_program program) CL_API_SUFFIX__VERSION_1_0 {
  shim::IntrusivePtr<shim::cl::Program> prog(static_cast<shim::cl::Program*>(program), false);
  return CL_SUCCESS;
}
...
SHIM_CALL(clBuildProgram)(cl_program           program,
  ...
  if (auto status = SHIM_CALL(clCompileProgram)(program, num_devices, device_list, options,
  ...
  shim::IntrusivePtr<shim::cl::Program> linked(static_cast<shim::cl::Program*>(
      SHIM_CALL(clLinkProgram)(prog->ctx_.get(), 1, devlist, nullptr,
                               1, proglist, pfn_notify, user_data, &status)), false);
...
SHIM_CALL(clLinkProgram)(cl_context           context,
  ...
  std::transform(input_programs, input_programs + num_input_programs,
                 ...
                 [&files](cl_program p) {
                   shim::IntrusivePtr<shim::cl::Program> prog(static_cast<shim::cl::Program*>(p));
                   return prog->object_;
                 });
  ...
  CmProgram *p = nullptr;
  if (ctx->dev_.device->LoadProgram(blob.data(), blob.size(), p) != CM_SUCCESS) {
    ERRCODE(CL_INVALID_BINARY);
    return nullptr;
  }
...
SHIM_CALL(clGetProgramBuildInfo)(cl_program            program,
  ...
  switch (param_name) {
    ...
    case CL_PROGRAM_BUILD_LOG:
      return SetResult(""sv, param_value_size, param_value,
                       param_value_size_ret);
      ...
      switch (prog->type_) {
        case shim::cl::Program::Type::kSource:
          return SetResult<cl_program_binary_type>(CL_PROGRAM_BINARY_TYPE_NONE,
                                                   ...
                                                   param_value_size_ret);
        case shim::cl::Program::Type::kObject:
          return SetResult<cl_program_binary_type>(CL_PROGRAM_BINARY_TYPE_COMPILED_OBJECT,
                                                   param_value_size, param_value,
                                                   param_value_size_ret);
        case shim::cl::Program::Type::kBinary:
          return SetResult<cl_program_binary_type>(CL_PROGRAM_BINARY_TYPE_EXECUTABLE,
                                                   param_value_size, param_value,
                                                   param_value_size_ret);
      }
